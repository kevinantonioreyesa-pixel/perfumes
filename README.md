...
def cargar_datos():
    try:
        ...
        st.error("Error: Uno o ambos archivos CSV (ebay_mens_perfume.csv, ebay_womens_perfume.csv) no fueron encontrados por la función cargar_datos().")
        return None
...
if df is None:
    ...
    # In a Colab notebook, st.stop() does not stop the Python interpreter.
    # To prevent the TypeError on subsequent lines, we must explicitly exit the cell execution.
    sys.exit("Execution stopped due to missing CSV files.") # This will halt the cell
